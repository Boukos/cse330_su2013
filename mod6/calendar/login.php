...
if(isset($_POST['username'])){
        ...
        $stmt1 = $mysqli->prepare("select count(*) from calendarusers where user_name like ?");
        if(!$stmt1) {
                printf("Failed: %s\n", $mysqli->error);
                exit;
        }
        ...
        if ($number == 0) {
                ...
                $stmt = $mysqli->prepare("select count(user_name), user_name, password from calendarusers where user_name=?"
);
                if(!$stmt){
