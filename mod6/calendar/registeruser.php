...
if(isset($_POST['newuser'])){
	...
	$stmt1 = $mysqli->prepare("select count(*) from calendarusers where user_name like '$username'");
	if(!$stmt1){
		printf("Failed: %s\n", $mysqli->error);
		exit;
	}
	...
	if($number > 0){
		...
		if(strcmp($password, $repeated) == 0){
			$stmt = $mysqli->prepare("insert into calendarusers(user_name, password) values(?, ?)");
			if(!stmt){
